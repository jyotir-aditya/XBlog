...
export default function Page({slug}) {
  ...
  async function onSubmit(event) {
    ...
    if (session) {
      ...
      console.log("sumbiting selected category",selectedCategory,title,postId);
  ...
  useEffect(() => {
    ...
    async function getData(slug) {
        ...
        setCategories(result.map((category) => ({value: category.id,label: category.name, })));
  ...
  useEffect(() => {
    ...
    setSelectedCategory(categories.find((cat) => cat.value === postData.category_id))
    }
  }, [categories])
  ...
  return (
    <div className="min-h-[100vh] h-fit w-full flex justify-center items-center">
      <div className="mt-[10vh]">
        {postData&&categories&&<div className="Form w-full h-full max-w-[60vw] min-w-[50vw] bg-white shadow-md rounded-xl mb-[2vh] p-[2vw]">
            ...
            <div className={`${isVisible ? "hidden" : "block"} h-fit gap-4 space-y-6`}>
              ...
              <div className="flex flex-col">
                <Select
                  ...
                  defaultValue={categories.find((cat) => cat.value === postData.category_id)}
