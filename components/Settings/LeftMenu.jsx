...
const LeftMenu = () => {
  ...
  const renderMenuItem = (label, isClickable) => (
    <div
      className={`text-xl font-robo py-2 pl-2 flex font-medium items-center justify-between cursor-${
        isClickable ? "pointer" : "not-allowed"
      } ${isSelected ? "sm:bg-slate-100" : ""}`}
  ...
  return (
    <div className="LeftMenu w-full sm:w-[30vw] border-r-2 h-full border-t relative">
      <div className="text-3xl sm:text-5xl font-medium font-robo mt-[9vh]">
        Settings
      </div>
      <div className="Content flex flex-col gap-4 mt-[5vh] sm:ml-[2vw]">
        ...
        <Link href={"/get_premium"}>{renderMenuItem("Membership",true)}</Link>
      ...
      {hoveredItem && (
        <div className="fixed top-16 -translate-x-[5vw] w-[100vw] flex justify-center">
          <div className="bg-gray-800 text-white text-sm p-2  rounded shadow-lg">
            {hoveredItem} is currently under development.
          </div>
        </div>
      )}
    </div>
